...
class CalendarYvv {
  constructor(etiqueta = "", diaSeleccionado = "", primerDia = "Lunes") {
    ...
    this.diaSeleccionado =
      diaSeleccionado == "" ? moment().format("Y-M-D") : diaSeleccionado; // día actual seleccionado
  ...
  createHeaderM() {
    ...
    var title = $("<span class='text-uppercase'>").html(
      moment(this.diaSeleccionado).format("MMMM - Y")
    );
    var _this = this;
  ...
  createDayTag() {
    ...
    var cont = $(
      "<div class='d-flex border w-100 border-top-0 " +
        this.bg +
        " " +
        this.textColor +
        "'>"
    ...
    diasOrd.fechCort.forEach(function(e) {
      var div = $(
        "<div class='d-flex border flex-fill w-100 justify-content-center p-2'>"
      ).html(e);
      cont.append(div);
    });
    return cont;
  ...
  createDaysMont() {
    var diaSelected = this.corregirMesA(
    ...
    for (var i = 0; i < this.cantDias; i++) {
      var fechNow =
        this.anioSeleccionado + "-" + this.mesSeleccionado + "-" + (i + 1);
      var div = $(
        "<div class='d-flex border flex-fill w-100 justify-content-center pt-3 pb-3 btn " +
          this.btnD +
          "' data-date='" +
          fechNow +
          "'>"
      ...
      if (this.diaSeleccionado == fechNow) {
        div = $(
          "<div class='current-date-selected d-flex border flex-fill w-100 justify-content-center pt-3 pb-3 btn " +
            this.btnD +
            "' data-date='" +
            fechNow +
            "'>"
  ...
  mesAnterior(ev) {
    ...
    ev.diaSeleccionado =
      ev.anioSeleccionado + "-" + ev.mesSeleccionado + "-" + day;
